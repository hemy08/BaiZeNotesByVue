...
### 1.1 设计目标
白泽笔记的设计目标是创建一个功能强大、易于使用、视觉美观的Markdown编辑器。设计遵循"简洁而不简单"的理念，在保持界面清爽的同时，提供丰富的功能和良好的用户体验。本项目基于 Electron 38.0.0 + Vue 3.4.27 + TypeScript 6.0.2 技术栈构建，集成 Monaco Editor 0.55.0 作为核心编辑器，提供专业级的代码编辑体验。
...
```
┌─────────────────────────────────────────────────────────┐
│  标题栏 (自定义标题栏，支持拖拽、最小化、最大化、关闭)        │
├─────────────────────────────────────────────────────────┤
│  菜单栏 (自定义菜单栏，集成系统菜单功能)                     │
├──────┬──────────────────────────────────────────────────┤
│      │  工具栏 (格式化、插入、切换视图等快捷操作)            │
│ 文   ├──────────────────────────────────────────────────┤
│ 件   │                                                   │
│ 树   │  编辑区 (Monaco Editor 0.55.0)                     │
│      │  - 语法高亮                                        │
│      │  - 实时预览                                        │
│      ├──────────────────────────────────────────────────┤
│      │  预览区 (Markdown实时预览)                         │
├──────┴──────────────────────────────────────────────────┤
│  状态栏 (实时反馈：文件状态、光标位置、字数统计等)            │
└─────────────────────────────────────────────────────────┘
```
...
- **可折叠**: 文件树和预览区可折叠
- **响应式**: 根据窗口大小自动调整
- **自定义标题栏**: 无边框窗口设计，自定义标题栏支持拖拽移动、双击最大化/还原、8方向窗口调整
- **自定义菜单栏**: 集成系统菜单功能，支持键盘快捷键
- **状态栏实时反馈**: 显示文件状态、光标位置、字数统计、编码格式等信息

### 2.2 组件架构

#### 2.2.1 组件统计
项目包含 **65个Vue组件**，采用组件化设计理念，实现高复用性和可维护性。

#### 2.2.2 核心组件分类
- **布局组件**: TitleBar, MenuBar, StatusBar, Sidebar等
- **编辑器组件**: MonacoEditor, MarkdownPreview, CodeEditor等
- **文件管理组件**: FileTree, FileExplorer, FileTab等
- **工具组件**: Toolbar, PluginPanel, ThemeSelector等
- **对话框组件**: SettingsDialog, AboutDialog, SearchDialog等
...
**设计理念**: 提供丰富的快捷键支持，提升用户操作效率，支持键盘优先的操作模式。
...
interface ThemeConfig {
  appTheme: string        // 应用主题名称
  editorTheme: string     // 编辑器主题名称
...
// 主题切换流程
async function switchTheme(config: ThemeConfig) {
  // 1. 应用应用主题
  await applyAppTheme(config.appTheme)

  // 2. 应用编辑器主题
  await applyEditorTheme(config.editorTheme)

  // 3. 保存配置
  await saveThemeConfig(config)

  // 4. 发送主题变更事件
  emit('theme-changed', config)
}
```
...
```typescript
// 文件选择
ipcMain.on('baize:notes:open-select-file', (event, options) => {
  // 显示文件选择对话框
  // 返回选中的文件路径
})
...
async function loadLargeFile(filePath: string) {
  ...
  if (fileSize > strategy.threshold) {
    // 使用分块加载
    return loadInChunks(filePath, strategy.chunkSize)
  } else {
    // 正常加载
...
// 应用场景
const fileContentCache = new LRUCache<string, string>(50)  // 缓存50个文件
const renderedHTMLCache = new LRUCache<string, string>(100)  // 缓存100个渲染结果
```
...
```typescript
// 并行渲染策略
async function parallelRender(content: string) {
  // 将内容分割为多个部分
  const sections = splitContent(content)

  // 使用Web Worker并行渲染
  const workers = [
    new Worker('markdown-renderer.js'),
    new Worker('mermaid-renderer.js'),
    new Worker('katex-renderer.js')
  ]

  // 并行处理
  const results = await Promise.all([
    workers[0].render(sections.markdown),
    workers[1].render(sections.diagrams),
    workers[2].render(sections.formulas)
  ])
...
- **2026-04-30**: 更新至 v1.1.5-beta 版本，更新技术栈信息（Electron 38.0.0 + Vue 3.4.27 + TypeScript 6.0.2），集成 Monaco Editor 0.55.0，新增主题系统设计（20+应用主题 + 55编辑器主题），完善性能优化设计（大文件优化、LRU缓存、并行渲染、内存管理），更新组件架构（61个Vue组件）
...
class PluginManager {
  ...
  // 插件卸载
  async unloadPlugin(pluginId: string): void
